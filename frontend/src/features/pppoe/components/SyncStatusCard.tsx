...
interface SyncStatusCardProps {
  syncResult: PppoeSyncResult | null;
  isLoading: boolean;
  onAudit: () => void;
  onRepair: () => void;
  canSync: boolean;
  ...
  if (!syncResult) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm flex items-center justify-between">
        <div>
          <h3 className="text-base font-semibold text-slate-900">Router Synchronization Status</h3>
          <p className="text-sm text-slate-500 mt-1">Run an audit to check for profile mismatches or missing secrets on MikroTik routers.</p>
        </div>
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Checked at {syncResult.checked_at ?? 'just now'}. SkyFi DB: <strong className="text-slate-800">{syncResult.total_accounts_in_db}</strong> accounts | RouterOS secrets: <strong className="text-slate-800">{syncResult.total_secrets_on_router}</strong>.
          </p>
        </div>
        <div className="flex gap-2 shrink-0">
          {canSync ? (
            <>
              <Button variant="secondary" onClick={onAudit}>Re-audit</Button>
              {!isSynced ? <Button onClick={onRepair} className="bg-amber-600 hover:bg-amber-700">Repair Conflicts</Button> : null}
            </>
          ) : null}
        </div>
      </div>
      ...
      {!isSynced && syncResult.discrepancies.length > 0 ? (
        <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50/50 p-4 space-y-2 max-h-64 overflow-y-auto">
          <p className="text-xs font-semibold uppercase tracking-wider text-amber-800">Identified Discrepancies ({syncResult.discrepancies.length})</p>

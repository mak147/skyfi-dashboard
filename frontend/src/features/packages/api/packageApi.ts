...
export async function exportPackages(filters: PackageFilters = {}) { const params = new URLSearchParams(); Object.entries(filters).forEach(([key, value]) => { if (value) params.set(`filter[${key}]`, value) }); const query = params.size ? `?${params.toString()}` : ''; const response = await apiClient.get(`/packages/export${query}`, { responseType: 'blob' }); const url = URL.createObjectURL(response.data); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'skyfi-packages.csv'; anchor.click(); URL.revokeObjectURL(url) }

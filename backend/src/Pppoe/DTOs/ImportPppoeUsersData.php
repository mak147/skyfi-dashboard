...
final class ImportPppoeUsersData
{
    ...
    {
        $rawUsernames = is_array($data['usernames'] ?? null) ? $data['usernames'] : [];
        $cleanUsernames = [];
        foreach ($rawUsernames as $u) {
            $trimmed = trim((string) $u);
            if ($trimmed !== '') {
                $cleanUsernames[] = $trimmed;
            }
        }
        ...
        return new self(
            ...
            overwriteConflicts: filter_var($data['overwrite_conflicts'] ?? false, FILTER_VALIDATE_BOOLEAN),
        );
    }
}

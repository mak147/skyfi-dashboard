<?php

declare(strict_types=1);

namespace SkyFi\Hotspot\Controllers;

use SkyFi\Hotspot\Contracts\HotspotSyncLoggerContract;
use SkyFi\Hotspot\DTOs\ImportHotspotUsersData;
use SkyFi\Hotspot\DTOs\SyncOptionsData;
use SkyFi\Hotspot\Services\HotspotSyncService;
use SkyFi\Rbac\Middleware\RequirePermissionMiddleware;
use SkyFi\Shared\Http\Request;
use SkyFi\Shared\Http\Response;

final class HotspotSyncController
{
    public function __construct(
        private readonly HotspotSyncService $syncService,
        private readonly HotspotSyncLoggerContract $syncLogger,
        private readonly RequirePermissionMiddleware $authorizer,
    ) {
    }

    public function syncRouter(Request $request): Response
    {
        $actorId = $this->authorize($request, 'hotspot.sync');
        $routerId = (int) (($request->attributes()['route_params'] ?? [])['routerId'] ?? 0);

        $result = $this->syncService->syncRouter($routerId, $actorId);

        return new Response(200, [
            'data' => [
                'type' => 'hotspot-sync-results',
                'id' => (string) $routerId,
                'attributes' => $result->toArray(),
            ],
        ]);
    }

    public function syncUser(Request $request): Response
    {
        $actorId = $this->authorize($request, 'hotspot.sync');
        $userId = (int) (($request->attributes()['route_params'] ?? [])['id'] ?? 0);

        $result = $this->syncService->syncUser($userId, $actorId);

        return new Response(200, [
            'data' => [
                'type' => 'hotspot-sync-results',
                'id' => (string) $result->routerId(),
                'attributes' => $result->toArray(),
            ],
        ]);
    }

    public function detectMissing(Request $request): Response
    {
        $this->authorize($request, 'hotspot.sync');
        $query = $request->query();
        $routerId = isset($query['router_id']) && is_numeric($query['router_id']) ? (int) $query['router_id'] : null;

        $missing = $this->syncService->detectMissing($routerId);

        return new Response(200, [
            'data' => array_map(static fn ($m, $idx): array => [
                'type' => 'hotspot-missing-users',
                'id' => (string) ($idx + 1),
                'attributes' => $m,
            ], $missing, array_keys($missing)),
        ]);
    }

    public function repair(Request $request): Response
    {
        $actorId = $this->authorize($request, 'hotspot.sync');
        $options = SyncOptionsData::fromArray($request->body());

        $result = $this->syncService->repair($options, $actorId);

        return new Response(200, [
            'data' => [
                'type' => 'hotspot-sync-repairs',
                'id' => 'repair-' . time(),
                'attributes' => $result,
            ],
        ]);
    }

    public function importUsers(Request $request): Response
    {
        $actorId = $this->authorize($request, 'hotspot.sync');
        $data = ImportHotspotUsersData::fromArray($request->body());

        $result = $this->syncService->importUsers($data, $actorId);

        return new Response(200, [
            'data' => [
                'type' => 'hotspot-import-results',
                'id' => 'import-' . time(),
                'attributes' => $result,
            ],
        ]);
    }

    public function importProfiles(Request $request): Response
    {
        $actorId = $this->authorize($request, 'hotspot.sync');
        $body = $request->body();
        $routerId = (int) ($body['router_id'] ?? 0);

        $result = $this->syncService->importProfiles($routerId, $actorId);

        return new Response(200, [
            'data' => array_map(static fn ($p, $idx): array => [
                'type' => 'hotspot-imported-profiles',
                'id' => (string) ($p['id'] ?? $idx + 1),
                'attributes' => $p,
            ], $result, array_keys($result)),
        ]);
    }

    public function routerProfiles(Request $request): Response
    {
        $this->authorize($request, 'hotspot.view');
        $routerId = (int) (($request->attributes()['route_params'] ?? [])['routerId'] ?? 0);

        $profiles = $this->syncService->listRouterProfiles($routerId);

        return new Response(200, [
            'data' => array_map(static fn ($p, $idx): array => [
                'type' => 'mikrotik-hotspot-profiles',
                'id' => (string) ($p['id'] ?? $idx + 1),
                'attributes' => $p,
            ], $profiles, array_keys($profiles)),
        ]);
    }

    public function syncLogs(Request $request): Response
    {
        $this->authorize($request, 'hotspot.sync');
        $query = $request->query();
        $limit = isset($query['limit']) && is_numeric($query['limit']) ? (int) $query['limit'] : 50;
        $routerId = isset($query['router_id']) && is_numeric($query['router_id']) ? (int) $query['router_id'] : null;
        $userId = isset($query['user_id']) && is_numeric($query['user_id']) ? (int) $query['user_id'] : null;

        $logs = $this->syncLogger->listRecent($limit, $routerId, $userId);

        return new Response(200, [
            'data' => array_map(static fn ($l): array => [
                'type' => 'hotspot-sync-logs',
                'id' => (string) ($l['id'] ?? '0'),
                'attributes' => $l,
            ], $logs),
        ]);
    }

    private function authorize(Request $request, string $permission): int
    {
        $claims = $request->attributes()['claims'] ?? [];
        $userId = isset($claims['sub']) ? (int) $claims['sub'] : 0;
        $this->authorizer->authorize($userId, $permission);

        return $userId;
    }
}

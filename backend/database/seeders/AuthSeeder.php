...
final class AuthSeeder
{
    ...
    private function seedRolePermissions(PDO $connection, array $roleIds, array $permissionIds): void
    {
        $rolePermissions = [
            ...
            'Customer Support' => ['manage:ticket', 'view:customer', 'update:customer:contact', 'update:customer:notes', 'view:invoice', 'view:payment', 'create:payment:manual', 'view:service', 'view:network-status:customer', 'execute:service:reconnect', 'customers.view', 'customers.update', 'packages.view', 'payments.view', 'payments.create'],
            'Installation Team / Field Technician' => ['view:work-order:own', 'update:work-order:own', 'view:customer:contact_and_address', 'view:inventory:own-vehicle', 'execute:site-survey', 'execute:service:diagnostics', 'customers.view', 'packages.view'],
            'Network Engineer' => ['manage:tower', 'manage:mikrotik-router', 'view:ip-address-pool', 'execute:provisioning:manual', 'execute:config-backup', 'view:network-status:global', 'view:customer:network-details', 'view:report:network', 'customers.view', 'customers.manage', 'packages.view', 'packages.update', 'packages.manage'],
            'Inventory Manager' => ['manage:inventory-item', 'manage:warehouse', 'execute:stock-transfer', 'manage:vendor', 'manage:purchase-order', 'view:report:inventory', 'customers.view', 'packages.view'],
        ];
        $statement = $connection->prepare(
